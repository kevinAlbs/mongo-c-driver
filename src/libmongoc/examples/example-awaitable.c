...
void apm_command_started_cb (const mongoc_apm_command_started_t *event) {
   const char *cmd;

   cmd = mongoc_apm_command_started_get_command_name (event);
   MONGOC_DEBUG ("apm_command_started_cb: %s", cmd);
}
void apm_command_succeeded_cb (const mongoc_apm_command_succeeded_t *event) {
   const char *cmd;

   cmd = mongoc_apm_command_succeeded_get_command_name (event);
   MONGOC_DEBUG ("apm_command_succeeded_cb: %s", cmd);
}
void apm_command_failed_cb (const mongoc_apm_command_failed_t *event) {
   const char *cmd;

   cmd = mongoc_apm_command_failed_get_command_name (event);
   MONGOC_DEBUG ("apm_command_failed_cb: %s", cmd);
}
void apm_server_changed_cb (const mongoc_apm_server_changed_t *event) {
   MONGOC_DEBUG ("apm_server_changed_cb");
}
void apm_server_opening_cb (const mongoc_apm_server_opening_t *event) {
   MONGOC_DEBUG ("apm_server_opening_cb");
}
void apm_server_closed_cb (const mongoc_apm_server_closed_t *event) {
   MONGOC_DEBUG ("apm_server_closed_cb");
}
void apm_topology_changed_cb (const mongoc_apm_topology_changed_t *event) {
   MONGOC_DEBUG ("apm_topology_changed_cb");
}
void apm_topology_opening_cb (const mongoc_apm_topology_opening_t *event) {
   MONGOC_DEBUG ("apm_topology_opening_cb");
}
void apm_topology_closed_cb (const mongoc_apm_topology_closed_t *event) {
   MONGOC_DEBUG ("apm_topology_closed_cb");
}
void apm_server_heartbeat_started_cb (const mongoc_apm_server_heartbeat_started_t *event) {
   MONGOC_DEBUG ("apm_server_heartbeat_started_cb");
}
void apm_server_heartbeat_succeeded_cb (const mongoc_apm_server_heartbeat_succeeded_t *event) {
   MONGOC_DEBUG ("apm_server_heartbeat_succeeded_cb");
}
void apm_server_heartbeat_failed_cb (const mongoc_apm_server_heartbeat_failed_t *event) {
   MONGOC_DEBUG ("apm_server_heartbeat_failed_cb");
}

static mongoc_apm_callbacks_t* apm_callbacks_new (void) {
   ...
   mongoc_apm_set_server_heartbeat_started_cb (callbacks, apm_server_heartbeat_started_cb);
   mongoc_apm_set_server_heartbeat_succeeded_cb (callbacks, apm_server_heartbeat_succeeded_cb);
   mongoc_apm_set_server_heartbeat_failed_cb (callbacks, apm_server_heartbeat_failed_cb);

   return callbacks;
}

static void _fail_insert (mongoc_client_t *client, bool close_connection) {
   ...
   BCON_APPEND (&cmd,
      ...
      "mode", "{", "times", BCON_INT32(1), "}",
      "data", "{", "failCommands", "[", "insert", "]", "errorCode", BCON_INT32(10107), "closeConnection", BCON_BOOL (close_connection), "}"
   );
...
main (int argc, char **argv)
{
   ...
   uri = mongoc_uri_new ("mongodb://localhost:27017,localhost:27018/?replicaSet=rs0&retryWrites=true");
   ...
   /* Trigger an insert failure. Since it is a retryable write, this should still succeed.
