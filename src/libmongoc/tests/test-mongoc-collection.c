...
test_create_indexes_acts_as_write_command (void *unused)
{
   ...
   // Test write concern is inherited from collection:
   {
      ...
      // Create index:
      {
         ...
         ASSERT (!ok);                                      // TODO: test (expectedly) fails. Get passing.
         ASSERT_ERROR_CONTAINS (error, 123, 456, "foobar"); // TODO: set expected error.
         mongoc_index_model_destroy (im);
      }
   }
   ...
   {
      // TODO: set a failpoint to fail with "writeConcernError".
   }
