...
#define BSON_OBJECT_ALLOWANCE (16 * 1024)
           ...
           "begin\n");
   {
      ...
      ASSERT (!ok);
      ASSERT_ERROR_CONTAINS (
         error, MONGOC_ERROR_SERVER, 2, "object to insert too large");
           ...
           "BSON_OBJECT_ALLOWANCE ... begin\n");
   {
      ...
      bool ok = mongoc_collection_update_one (
         coll, &doc, tmp_bson ("{}"), NULL, NULL, &error);
      ASSERT_OR_PRINT (ok, error);
           ...
           "BSON_OBJECT_ALLOWANCE + 1 ... begin\n");
   {
      ...
      ASSERT_ERROR_CONTAINS (error,
                             MONGOC_ERROR_SERVER,
                             10334,
                             "Size must be between 0 and 16793600");
...
test_bulk_install (TestSuite *suite)
{
   ...
   TestSuite_AddLive (
      suite, "/BulkOperation/large_writes", test_bulk_large_writes);
}
