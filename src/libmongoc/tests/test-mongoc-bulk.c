...
test_bulk_large_writes (void)
{
   ...
   // 16777216 is copied from response of `hello` command on server
   // version 8.0.0-alpha-226-gdc49f69
   size_t bsonMaxObjSize = 16777216;
   // See: SERVER-10643
#define BSON_OBJECT_ALLOWANCE (16 * 1024)

   printf ("Test inserting document of size: bsonMaxObjSize ... begin\n");
   {
      bson_t doc = BSON_INITIALIZER;
      // Append an `_id` to include in size calculation.
      BSON_APPEND_INT32 (&doc, "_id", 1);

      size_t remaining = bsonMaxObjSize - doc.len;
      size_t overhead = 1   // BSON type
                        + 3 // key string
                        + 1 // key null terminator
                        + 4 // int32 value length
                        + 1 // value string
         ;
      size_t string_len = remaining - overhead;
      char *string = bson_malloc (string_len);
      memset (string, 'A', string_len);

      ASSERT (bson_append_utf8 (&doc, "foo", 3, string, string_len));
      ASSERT_CMPUINT32 (doc.len, ==, bsonMaxObjSize);
   ...
   printf ("Test inserting document of size: bsonMaxObjSize ... end\n");


   printf ("Test inserting document of size: bsonMaxObjSize + 1 ... "
           "begin\n");
   {
      bson_t doc = BSON_INITIALIZER;
      // Append an `_id` to include in size calculation.
      BSON_APPEND_INT32 (&doc, "_id", 2);
      ...
      size_t overhead = 1   // BSON type
                        ...
                        + 1 // value string
         ;
      size_t string_len = remaining - overhead + 1;
      char *string = bson_malloc (string_len);
      memset (string, 'A', string_len);
      string[string_len - 1] = '\0';

      ASSERT (bson_append_utf8 (&doc, "foo", 3, string, string_len));
      ASSERT_CMPUINT32 (doc.len, ==, bsonMaxObjSize + 1);

      bson_error_t error;
      bool ok = mongoc_collection_insert_one (coll, &doc, NULL, NULL, &error);
      ASSERT (!ok);
      ASSERT_ERROR_CONTAINS (error, MONGOC_ERROR_SERVER, 2, "object to insert too large");

      bson_free (string);
      bson_destroy (&doc);
   }
   printf ("Test inserting document of size: bsonMaxObjSize + 1 ... end\n");

   printf ("Test updating document of size: bsonMaxObjSize + "
           "BSON_OBJECT_ALLOWANCE ... begin\n");
   {
      bson_t doc = BSON_INITIALIZER;
      size_t string_len = bsonMaxObjSize + BSON_OBJECT_ALLOWANCE;
      // Subtract overhead.
      string_len -= 48;
      char *string = bson_malloc (string_len);
      memset (string, 'A', string_len);
      ...
      bson_error_t error;
      bool ok = mongoc_collection_update_one (coll, &doc, tmp_bson ("{}"), NULL, NULL, &error);
      ASSERT_OR_PRINT (ok, error);
   ...
   printf ("Test updating document of size: bsonMaxObjSize + "
           "BSON_OBJECT_ALLOWANCE ... end\n");

   printf ("Test updating document of size: bsonMaxObjSize + "
           "BSON_OBJECT_ALLOWANCE + 1 ... begin\n");
   {
      bson_t doc = BSON_INITIALIZER;
      size_t string_len = bsonMaxObjSize + BSON_OBJECT_ALLOWANCE;
      // Subtract overhead.
      ...
      bool ok = mongoc_collection_update_one (coll, &doc, tmp_bson ("{}"), NULL, NULL, &error);
      ASSERT (!ok);
      ASSERT_ERROR_CONTAINS (error, MONGOC_ERROR_SERVER, 10334, "Size must be between 0 and 16793600");
...
test_bulk_install (TestSuite *suite)
{
   ...
   TestSuite_AddLive (suite, "/BulkOperation/large_writes", test_bulk_large_writes);
}
