...
stream_tracker_initiator (const mongoc_uri_t *uri,
...
{
   BSON_ASSERT (!st->pool);   // Cannot track pool and single-threaded client.
   BSON_ASSERT (!st->client); // Can only track one single-threaded client.
   st->client = client;
...
stream_tracker_track_pool (stream_tracker_t *st, mongoc_client_pool_t *pool)
{
   ...
   BSON_ASSERT (!st->pool);   // Can only track one pool.
   BSON_ASSERT (!st->client); // Cannot track pool and single-threaded client.
...
static bool
tracked_streamimed_out (mongoc_stream_t *stream)
{
...
tracked_stream_new (mongoc_stream_t *stream, stream_tracker_t *st, const mongoc_host_list_t *host)
{
   ...
   ts->vtable.timed_out = tracked_streamimed_out;
