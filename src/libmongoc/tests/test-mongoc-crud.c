...
prose_test_11 (void *ctx)
{
   ...
   */
   ...
   ASSERT_MATCH (first, BSON_STR ({"nsInfo" : [ {"ns" : "db.coll"} ]}));
   // Expect the second only contains the namespace for `db.coll1`.
   bson_t *second = _mongoc_array_index (&captured, bson_t *, 1);
   ASSERT_MATCH (second, BSON_STR ({"nsInfo" : [ {"ns" : "db.coll1"} ]}));
...
{
   /*
   12 (proposed). `MongoClient.bulkWrite` returns error if cannot fit any operation
   */
   mongoc_client_t *client;
   BSON_UNUSED (ctx);
   bool ok;
   ...
   char *large_str;
   {
      large_str = bson_malloc0 (maxMessageSizeBytes + 1);
      memset (large_str, 'b', maxMessageSizeBytes);
   }
   ...
   // Execute.
   {
      ...
      ASSERT_ERROR_CONTAINS (error, MONGOC_ERROR_COMMAND, MONGOC_ERROR_COMMAND_INVALID_ARG, "unable to send document");
      mongoc_bulkwriteresult_destroy (bwr.res);
      mongoc_bulkwriteexception_destroy (bwr.exc);
   }
