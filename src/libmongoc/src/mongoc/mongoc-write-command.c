                     ...
                     bson_error_t *error)
{
   ...
   do {
      ...
      if (ship_it) {
         ...
         ret = mongoc_cluster_write_cmd (
            &client->cluster, &parts.assembled, is_retryable, &new_retry_server_stream, &reply, error);
         ...
         if (!ret) {
            ...
            if (command->flags.ordered || !mongoc_cluster_stream_valid (&client->cluster, server_stream)) {
               // CDRIVER-????: this `if` condition may be a bug. I expect `server_stream` is invalidated on a
               // successful retry. If a network error occurs, and a retry succeeds, I expect stopping is not needed.
