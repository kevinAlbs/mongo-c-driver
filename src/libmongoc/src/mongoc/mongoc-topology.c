                                  ...
                                  bson_error_t *error)
{
   ...
   for (;;) {
      ...
      if (selected_server) {
         MONGOC_DEBUG ("server selection found server");
      }
      if (!selected_server) {
...
/*
 * Update the topology from the response to a handshake on a new application connection.
 * Only applicable to a client pool (single-threaded clients reuse monitoring connections).
