...
// COPY_INTERNAL_BSON_FIELD copies a `bson_t` that references data in
// `last_hello_response`.
#define COPY_INTERNAL_BSON_FIELD(FIELD)                                        \
   if (1) {                                                                    \
      if (!bson_empty (&description->FIELD)) {                                 \
         ptrdiff_t offset = bson_get_data (&description->FIELD) -              \
                            bson_get_data (&description->last_hello_response); \
         const uint8_t *data =                                                 \
            bson_get_data (&copy->last_hello_response) + offset;               \
...
// COPY_INTERNAL_STRING_FIELD copies a `const char*` that references data in
// `last_hello_response`.
#define COPY_INTERNAL_STRING_FIELD(FIELD)                                 \
   if (1) {                                                               \
      if (description->FIELD) {                                           \
         ptrdiff_t offset =                                               \
            ...
            (char *) bson_get_data (&description->last_hello_response);   \
         copy->FIELD =                                                    \
            (char *) bson_get_data (&copy->last_hello_response) + offset; \
      } else {                                                            \
         copy->FIELD = NULL;                                              \
      }                                                                   \
   } else                                                                 \
   ...
   mongoc_server_description_t *copy =
      bson_malloc (sizeof (mongoc_server_description_t));

   COPY_FIELD (id);
   COPY_FIELD (host);
   if (description->has_hello_response) {
      copy->round_trip_time_msec = bson_atomic_int64_fetch (
   ...
   // `topology_version` does not refer to data in `last_hello_response`. It
   // needs to outlive `last_hello_response`.
