 ...
 */
bool
_ctx_check_error (mongocrypt_ctx_t *ctx, bson_error_t *error)
{
...
bool
_kms_ctx_check_error (mongocrypt_kms_ctx_t *kms_ctx, bson_error_t *error)
{
   mongocrypt_status_t *status;
...
bool
_crypt_check_error (mongocrypt_t *crypt, bson_error_t *error)
{
   mongocrypt_status_t *status;
...
static bool
_need_mongo_collinfo (mongoc_client_t *client,
                      const char *db_name,
...
static bool
_need_mongo_markings (mongoc_client_t *client,
                      mongocrypt_ctx_t *ctx,
                      bson_error_t *error)
{
   bool ret = false;
...
static bool
_need_mongo_keys (mongoc_client_t *client,
                  mongocrypt_ctx_t *ctx,
                  bson_error_t *error)
{
   bool ret = false;
             ...
             bson_error_t *error)
{
   ...
   /* The chosen port may change pending SPEC-1431 */
   ...
   if (!mongoc_stream_tls_handshake_block (tls_stream, endpoint, 1000, error)) {
      goto fail;
   }
...
static bool
_need_kms (mongoc_client_t *client, mongocrypt_ctx_t *ctx, bson_error_t *error)
{
   ...
   /* When NULL is returned, this can either be an error or end-of-list */
   if (!_ctx_check_error (ctx, error)) {
      goto fail;
   }
 ...
 */
bool
_mongoc_fle_run_state_machine (mongoc_client_t *client,
                               const char *db_name,
                               mongocrypt_ctx_t *ctx,
                               bson_t **result,
                               bson_error_t *error)
   ...
   *result = NULL;
   while (true) {
      ...
      case MONGOCRYPT_CTX_NEED_MONGO_COLLINFO:
         if (!_need_mongo_collinfo (client, db_name, ctx, error)) {
            goto fail;
         }
         break;
      case MONGOCRYPT_CTX_NEED_MONGO_MARKINGS:
         if (!_need_mongo_markings (client, ctx, error)) {
            goto fail;
         }
         break;
      case MONGOCRYPT_CTX_NEED_MONGO_KEYS:
         if (!_need_mongo_keys (client, ctx, error)) {
            goto fail;
         }
         break;
      case MONGOCRYPT_CTX_NEED_KMS:
         if (!_need_kms (client, ctx, error)) {
            goto fail;
         }
         break;
      case MONGOCRYPT_CTX_READY:
         if (!_ready (client, ctx, result, error)) {
            goto fail;
         }
         break;
 ...
 *    If @cmd contains a type=1 payload, convert it into a type=0 payload.
                          ...
                          bson_error_t *error)
{
   ...
   if (!_mongoc_fle_run_state_machine (
                          ...
                          bson_error_t *error)
{
   ...
   if (!_mongoc_fle_run_state_machine (client, db_name, ctx, &result, error)) {
      goto fail;
   }
