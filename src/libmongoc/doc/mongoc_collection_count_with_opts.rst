...
Use :symbol:`mongoc_collection_count_documents` or :symbol:`mongoc_collection_estimate_document_count` instead.
...
:symbol:`mongoc_collection_estimate_document_count` has the same performance as calling :symbol:`mongoc_collection_count_with_opts` with a NULL ``query``, but is not guaranteed to retrieve an accurate collection count.
