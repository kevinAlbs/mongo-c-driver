...
Automatic Client-Side Encryption is enabled by creating calling :symbol:`mongoc_client_enable_auto_encryption()` on a :symbol:`mongoc_client_t`. The following examples show how to setup automatic client-side field level encryption using :symbol:`mongoc_client_encryption_t` to create a new encryption data key.
...
The following example shows how to setup automatic client-side field level encryption using :symbol:`mongoc_client_encryption_t` to create a new encryption data key and create a collection with the Automatic Encryption JSON Schema Syntax:
