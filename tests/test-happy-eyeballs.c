...
typedef struct he_testcase_expected {
   ...
   /* { "ipv4", "ipv6", "both" }. Which servers the client tries to connect. */
   char *tries_cxn_to;
...
test_happy_eyeballs_testcase_setup (he_testcase_t *testcase)
{
   ...
   for (i = 0; i < 2; i++) {
      ...
      if (strcmp (server->type, "ipv4") == 0) {
         ...
         printf ("ipv4 mock server listening on port %d\n", port);
      ...
      if (strcmp (server->type, "ipv6") == 0) {
         ...
         mock_server_run (mock_server);
         port = mock_server_get_port (mock_server);
         printf ("ipv6 mock server listening on port %d\n", port);
         testcase->state.mock_servers[i] = mock_server;
      }
   }
   return port;
...
test_happy_eyeballs ()
{
   const int e = 100; /* epsilon. Wiggle room expected time. */
   int i, ntests;
   ...
   he_testcase_t testcases[] = {
      ...
      /* my old implementation failed the following. Poll will block for connect
       * timeout ms, but should have timed out for a max of 250ms past the start
       * time. */
      {
         /* client. */
         {"both"},
         /* server sockets. */
         {{"ipv4", 0, false}, {"ipv6", 500, false}},
         /* expected. */
         {"ipv4", "ipv4", 2, 250, 250 + e},
      }
   };
