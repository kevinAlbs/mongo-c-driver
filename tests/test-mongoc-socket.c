...
static void
test_mongoc_socket_nonblocking_poll_refusal(void* ctx) {
	mongoc_stream_poll_t* poller;
	mongoc_socket_t* sock;
	mongoc_stream_socket_t* ssock;
	...
	/* TODO: is connect just giving us some default bad cxn?*/
	mongoc_socket_connect(sock, &ipv4_addr, sizeof(ipv4_addr), 0);
...
test_socket_install(TestSuite *suite)
{
	...
	TestSuite_AddFull(suite,
		"/Socket/poll_refusal",
		test_mongoc_socket_nonblocking_poll_refusal,
		NULL,
		NULL,
		test_framework_skip_if_slow);
}